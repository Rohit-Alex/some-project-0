...
import { dataClassificationService } from '../services'
import type { DataClassificationListParams } from '../types'

export const QUERY_KEYS = {
  list: 'dataClassificationEvents',
} as const

interface UseDataClassificationEventsParams extends Omit<DataClassificationListParams, 'startDate' | 'endDate'> {
  timeRangeKey: string // Use stable key like "24h" or "custom:start:end"
  startDate?: string
  endDate?: string
}

export function useDataClassificationEvents(params: UseDataClassificationEventsParams) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      dataClassificationService.getList({
        page,
        limit,
        sortBy,
        sortDirection,
...
export function useExportDataClassificationEvents() {
  const exportData = async () => {
    ...
    a.href = url
    a.download = `data-classification-events-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
