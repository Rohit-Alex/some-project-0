...
export default function ColumnFilterCell({ columnId, filter, value, onChange }: ColumnFilterCellProps) {
    ...
    const handleApply = () => {
        switch (filter.type) {
            case 'text': onChange(columnId, tempTextValue);
                break;
            case 'select': onChange(columnId, tempSelectValue);
                break;
            case 'date': onChange(columnId, tempDateValue ? tempDateValue.format('YYYY-MM-DD') : '');
                break;
            case 'dateRange':
                onChange(columnId, {
    ...
    const renderFilterContent = () => {
        switch (filter.type) {
            case 'text':
                return (
                    <TextField size="small" placeholder={filter.placeholder || 'Filter...'} value={tempTextValue} onChange={(e) => setTempTextValue(e.target.value)} fullWidth autoFocus />
                );
            ...
            case 'date':
                return (
                    <DatePicker label={filter.placeholder || 'Select date'} value={tempDateValue} onChange={(newValue) => setTempDateValue(newValue)} slotProps={{ textField: { size: 'small', fullWidth: true } }} />
                );

            case 'dateRange':
                return (
                    <Stack spacing={2}>
                        <DatePicker label="Start Date" value={tempDateStart} onChange={(newValue) => setTempDateStart(newValue)} slotProps={{ textField: { size: 'small', fullWidth: true } }} />
                        <DatePicker label="End Date" value={tempDateEnd} onChange={(newValue) => setTempDateEnd(newValue)} slotProps={{ textField: { size: 'small', fullWidth: true }}} />
                    </Stack>
                );
    ...
    return (
        <>
            <IconButton size="small" onClick={handleOpenPopover} sx={{ color: hasValue ? 'primary.main' : 'action.active' }}>
                <Badge color="primary" variant="dot" invisible={!hasValue}>
                    <FilterListOutlined fontSize="small" />
                </Badge>
            </IconButton>

            <Popover open={Boolean(anchorEl)} anchorEl={anchorEl} onClose={handleCancel} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }} transformOrigin={{ vertical: 'top', horizontal: 'left' }}>
